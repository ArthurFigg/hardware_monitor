...
class CartaoRecurso(ctk.CTkFrame):
    def __init__(
        ...
        **kwargs,
        ...
        self._label_descricao = ctk.CTkLabel(
            self,
            text=self._descricao_fn(Status.NORMAL),
            wraplength=240,
            justify="left",
    ...
    def atualizar(self, status: Status, valor: float = 0.0) -> None:
        self._status_atual = status
        self._semaforo.atualizar(status)
        self._label_descricao.configure(text=self._descricao_fn(status))
